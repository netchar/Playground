...
class HomeAdapter : RecyclerView.Adapter<HomeAdapter.ViewHolder>() {
    private val mutableItems = mutableListOf<Author>()

    fun submitItems(items: List<Author>) {
        mutableItems.clear()
    ...
    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(model: Author) {
            Glide.with(itemView.context)
                .load(model.photoUrl)
                .into(photoView)

            titleView.text = model.fullName
            descriptionView.text = model.email
        }
    }
}